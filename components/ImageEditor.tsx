...
const ImageEditor: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const mimeType =
        selectedImage.match(/data:([^;]+);/)?.[1] || 'image/jpeg';
      const result = await editImage(selectedImage, prompt, mimeType);
      setGeneratedImage(result);
    } catch (error) {
      console.error(error);
      alert(
        'Failed to edit image. Please ensure your API key supports this model.'
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="h-full p-6 lg:p-8 flex flex-col gap-6 overflow-y-auto">
        ...
        <div className="relative z-10">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2 flex items-center gap-3">
            <span className="w-10 h-10 rounded-lg bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-purple-500/20">
              ...
              >
                <path d="M11 21.73a2 2 0 0 0 2.94 0l7-7a2 2 0 0 0 2.12-2.94L16.49 5a2 2 0 0 0-2.61-.06l-6.26 6.27a2 2 0 0 0 0 2.82L11 17.5" />
                <path d="m15.5 10.5 2.18-2.18a.5.5 0 0 1 .7 0l2.64 2.64a.5.5 0 0 1 0 .7L18.82 13.82" />
                <path d="M2 2l19.8 19.8" />
                <path d="M15 5.88 5.88 15" />
              </svg>
            </span>
            Magic Image Editor
          </h1>
          <p className="text-gray-600 dark:text-gray-400 max-w-xl">
            Upload an image and use natural language to transform it using
            Gemini 2.5 Flash Image. Describe changes like "make it cyberpunk" or
            "remove background".
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-6 min-h-[400px]">
        ...
        <div className="flex-1 glass-panel rounded-2xl p-4 flex flex-col relative group">
          ...
          <div
            className={`flex-1 min-h-[300px] border-2 border-dashed border-gray-300 dark:border-gray-700/50 rounded-xl flex items-center justify-center overflow-hidden bg-gray-100/50 dark:bg-gray-900/30 transition-all duration-300 ${
              !selectedImage
                ? 'cursor-pointer hover:border-purple-500/50 hover:bg-gray-200/50 dark:hover:bg-gray-800/30'
                : ''
            }`}
          ...
          <div className="flex-1 min-h-[300px] bg-gray-100/50 dark:bg-gray-900/50 rounded-xl flex items-center justify-center overflow-hidden border border-gray-200 dark:border-white/5 relative shadow-inner">
            {loading ? (
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-purple-500 dark:text-purple-300 text-sm animate-pulse font-medium tracking-wide">
                  Magic in progress...
                </p>
              </div>
            ) : generatedImage ? (
