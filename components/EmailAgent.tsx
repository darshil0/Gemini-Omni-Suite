...
const EmailAgent: React.FC = () => {
  ...
  const getFolderConfig = (folderRaw: string | null) => {
      ...
      return {
        ...
        icon: (
          ...
          >
            <path d="M12 9v4" />
            <path d="M12 17h.01" />
            <path d="M3.34 19a2 2 0 0 0 1.66 1h14a2 2 0 0 0 1.66-1L12 2.66Z" />
          </svg>
        ),
      };
    if (folder.includes('action'))
      return {
    ...
    if (folder.includes('marketing') || folder.includes('promo'))
      return {
        label: 'Promotions',
    ...
    if (folder.includes('reference') || folder.includes('info'))
      return {
        label: 'Reference',
    ...
    if (folder.includes('personal') || folder.includes('social'))
      return {
        label: 'Personal',
    ...
    if (folder.includes('archive') || folder.includes('trash'))
      return {
        label: 'Archive',
  ...
  const recommendedFolder = analysis
    ? analysis.match(/Recommended Folder:\s*(.+)/i)?.[1]?.trim() || null
    : null;
          ...
          <div className="flex-1 overflow-y-auto p-6 scroll-smooth custom-scrollbar bg-gray-50/50 dark:bg-gray-900/20">
            {analysis ? (
              <div className="flex flex-col gap-6">
                ...
                {folderConfig && (
                  <div
                    className={`p-4 rounded-xl border flex items-center gap-4 ${folderConfig.colorClass}`}
                  >
